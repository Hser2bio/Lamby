...
#if defined(NDEBUG)
#error "lamby cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ulamby) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateLambySupply(int nHeightStart)
{
	AssertLockHeld(cs_main);
	...
	uiInterface.ShowProgress(_("Recalculating Lamby supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating Lamby supply..."), percent);
		}
...
CAmount GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        nSubsidy = 50000 * COIN;    // premine 50k
...
{
    util::ThreadRename("lamby-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> lambyInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                lambyInputs.push_back(stakeIn);
        }
        const bool hasLambyInputs = !lambyInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasLambyInputs) {
                    ...
                    for (const CTxIn& lambyIn : lambyInputs)
                        if(lambyIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasLambyInputs) {
                            for (const CTxIn& stakeIn : lambyInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // lamby: We use certain sporks during IBD, so check to see if they are
